...
class CourierStatusUpdated implements ShouldBroadcast
{
    ...
    {
        $courierId = $this->resolveCourierId($order);

        $orders = $courierId !== null
            ? $this->getActiveOrders($courierId)
                ->map(
                    fn (Order $activeOrder): array =>
                    $this->mapOrder($activeOrder)
                )
                ->values()
                ->all()
            : [];
    ...
    private function resolveCourierId(Order $order): ?int
    {
        if (
            in_array(
                $order->status,
