        ...
        <div class="max-w-[92rem] mx-auto sm:px-6 lg:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-12 gap-6">
                ...
                <div class="lg:col-span-7 space-y-6">
                    ...
                    <!-- Card 3: Process Photos -->
                    @if($order->photos && $order->photos->count())
                        <div class="bg-white rounded-3xl border border-gray-100 p-6 shadow-sm space-y-6">
                            <div class="flex items-center gap-3">
                                <div
                                    class="w-8 h-8 rounded-lg bg-teal-500 flex items-center justify-center text-white shadow-sm">
                                    <span class="material-symbols-outlined text-lg">photo_camera</span>
                                </div>
                                <div>
                                    <span class="text-[9px] font-black text-gray-400 uppercase tracking-widest">Visual
                                        Evidence</span>
                                    <h3 class="text-lg font-black text-gray-800 tracking-tight mt-0.5">Washing & Handling
                                        Photos</h3>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-2 sm:grid-cols-3 gap-4">
                                @foreach($order->photos as $photo)
                                    <div class="rounded-2xl overflow-hidden border border-gray-100 shadow-sm group relative bg-gray-50 cursor-pointer"
                                        onclick="openPhotoZoom('{{ Storage::url($photo->photo_path) }}')">
                                        <img src="{{ Storage::url($photo->photo_path) }}"
                                            class="w-full aspect-video object-cover transition-transform group-hover:scale-105 duration-300"
                                            alt="Process Proof">
                                        <div
                                            class="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center text-white text-xs font-black uppercase tracking-wider">
                                            <span class="material-symbols-outlined text-lg mr-1">zoom_in</span> View
                                        </div>
                                        <div class="p-2.5 bg-white border-t border-gray-50 flex items-center justify-between">
                                            <span
                                                class="text-[9px] font-black uppercase text-blue-600 bg-blue-50 px-2 py-0.5 rounded border border-blue-100">
                                                {{ str_replace('_', ' ', $photo->context) }}
                                            </span>
                                            <span
                                                class="text-[8px] text-gray-400 font-bold">{{ $photo->created_at->format('H:i') }}</span>
                                        </div>
                                    </div>
                                @endforeach
                            </div>
